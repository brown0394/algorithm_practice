...
bool fitting(vector<vector<int>>& game_board, vector<vector<int>>& table,
    ...
    if (gCol == tRow && gRow == tCol) {
        ...
        for (int i = 0; i <= row; ++i) {
            for (int j = 0; j <= col; ++j) {
                if (game_board[gbInfos[gidx].startI + i][gbInfos[gidx].startJ + j] != table[tbInfos[tidx].startJ + i][tbInfos[tidx].startJ + j]) {
                    found = false;
                    break;
                }
            }
...
int solution(vector<vector<int>> game_board, vector<vector<int>> table) {
    ...
    for (auto it = gbInfos.begin(); it != gbInfos.end(); ++it) {
        printf("%d, %d %d, %d %d\n", it->blocks, it->startI, it->startJ, it->endI
              , it->endJ);
    }
    return answer;
}